...
@pytest.mark.xdist_group(name='serial')
def test_a_backtest_matches_the_api(cli_runner, saved_dataloader, tmp_path):
...
def test_nothing_has_to_be_written_down_first(cli_runner, offline_dataloader):
    """Test discovery needs no file of any kind, only what it is told."""
...
def test_a_source_that_needs_a_key_reads_it_from_the_environment(cli_runner, monkeypatch):
    ...
    result = cli_runner.invoke(
        main,
        ['dataloader', 'params', '--league', 'NBA', '--stats', 'nba', '--odds', 'odds-api', '--odds-market', 'h2h'],
    )
    assert 'needs a key' not in result.output


@pytest.mark.xdist_group(name='serial')
def test_a_missing_key_says_which_one_is_missing(cli_runner, monkeypatch, tmp_path):
...
@pytest.mark.xdist_group(name='serial')
def test_a_model_of_your_own_is_named_by_where_it_lives(cli_runner, saved_dataloader, tmp_path):
    ...
    result = cli_runner.invoke(
        main,
        ['evaluation', 'backtest', '--dataloader', str(saved_dataloader), '--model', f'{models}:BETTOR', '--cv', '2'],
    )
    assert result.exit_code == 0, result.output
