...
is no configuration file to write first. `--stats` and `--odds` say where the data comes from, and `--download` fetches
it. A model of your own is Python, so you build it and name it: `--model models.py:BETTOR`.

```bash
# Extract training data
sportsbet data training --stats football-data --odds football-data \
  --league Italy --division 1 --year 2024 --odds-type market_maximum --download
