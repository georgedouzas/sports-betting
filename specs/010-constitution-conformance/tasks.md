...
- [ ] T051 [US4] Decide per name which of the 192 public names can run without a credential or the network,
...
- [ ] T055 [US4] Remove any example that cannot run from `src/sportsbet/execution/`, and move what it showed into the
      execution guide under `docs/` as reference code
- [ ] T056 [P] [US4] Confirm every code block under `docs/` executes at build and carries no hand-written output
- [ ] T058 [US4] Break one example on purpose, confirm `pdm run tests` goes red, then restore it
- [ ] T059 [US4] Run `pdm run formatting`, `pdm run checks`, `pdm run docs build`, then `pdm run tests`, and
      confirm the suite passed with no test in `tests/` edited

**Progress**: 9 examples written and running, on `format_event_time`, `parse_event_time`, `derive_market_base`,
`find_latest_odds_column`, `normalize_team_name`, `count_common_prefix`, `measure_names_similarity`, `pair_rosters`,
and `read_csv_content`. 26 documented surface names still carry none, and each needs the offline decision in T051:
5 exception classes, 14 execution names that need a venue or a credential, `fetch_payloads` which reads the feed, and
`load_object`, `build_dataloader`, `build_bettor`, `build_roster`, `normalize_identity`, which need a file or a frame
to exist first.

**Checkpoint**: not reached. T051 is the open decision
...
- [ ] T069 Run [quickstart.md](./quickstart.md) end to end and confirm every symptom in its regression table is
      absent
- [ ] T070 Run `pdm run clean` then the full gate, since a reused `nox` environment can carry a stale toolchain
