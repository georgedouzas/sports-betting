...
- [X] T050 [US1] The `data` branch was deleted by the maintainer, along with the Prefect flow. 919 odds CSVs are no longer published.
- [X] T051 [US1] Verified SC-001: the remote holds no `data` branch, and the only odds in the tree are 13 hand-authored rows for the offline dummy loader.
...
- [X] T066 [P] Run the full [quickstart.md](quickstart.md) validation, all eight scenarios
- [X] T067 [P] Confirm docstring coverage (`interrogate`) and that no network-touching class gained an executable doctest
- [X] T068 Confirm exactly one new runtime dependency was added (SC-012)
