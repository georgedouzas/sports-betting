"""Read the football-data.co.uk feed shared by its statistics and odds sources."""
...
def _to_snapshots(modelling: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Explode a wide modelling frame into long `stats` and `odds`, an unplayed match keeping only pre-play."""
    ...
    preplay = modelling[IDENTITY_COLS + feature_cols].assign(event_status='preplay', event_time=0)
    ...
    if {'home_half_goals', 'away_half_goals'}.issubset(modelling.columns):
        ...
        inplay = modelling.loc[half_mask, IDENTITY_COLS].assign(
    ...
    postplay = modelling.loc[full_mask, IDENTITY_COLS].assign(
...
def _extract_season_years(data: pd.DataFrame) -> pd.Series:
...
def _derive_kickoff(data: pd.DataFrame, date_format: str) -> pd.Series:
    """Return the kick-off instant in UTC, converted from the feed's UK time, midnight when absent."""
...
def _process_training(payloads: list[RawPayload]) -> list[tuple[str, int, int, pd.DataFrame]]:
    ...
    for payload in payloads:
        ...
        data['Date'] = _derive_kickoff(data, 'mixed')
        league, division, year = _parse_key_params(payload.item.key)
        if year is not None:
            ...
            data['year'] = _extract_season_years(data)
...
def _build_modelling_snapshots(payloads: list[RawPayload], catalogue: list) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if fixtures_payloads:
        fixtures = _process_fixtures(fixtures_payloads[0].content, _find_latest_years(catalogue, processed))
    modelling = _build_modelling_frames(processed, fixtures)
    ...
    def _combine(train: list[pd.DataFrame], upcoming: list[pd.DataFrame]) -> pd.DataFrame:
        """Return the training and upcoming frames as one."""
        frames = list(train)
        if upcoming:
...
class FootballDataSource(BaseSource):
    ...
    def read_catalogue(self: Self, payloads: list[RawPayload]) -> list[dict]:
        ...
        for payload in payloads:
            ...
            if key.startswith(f'{INDEX_PREFIX}_'):
                ...
                data = read_csv_content(payload.content)
                catalogue.extend(
                    (league, 1, int(year), _build_history_url(league))
                    for year in sorted(_extract_season_years(data).unique())
                )
    ...
    def list_required_items(self: Self, params: list[dict], schedule: pd.DataFrame | None = None) -> list[RawItem]:
        ...
        for param in params:
            ...
            if url is None:
                continue
            item = self._build_season_item(param['league'], param['division'], param['year'], url)
